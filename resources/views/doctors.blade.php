<!DOCTYPE html>
 ...
 <script>
          ...
          modalContentHtml = `
            ...
            <div id="signatureCanvasContainer" class="hidden mb-4">
              ...
              <div id="cameraInputContainer" class="hidden mb-4">
                  ...
                  <canvas id="signatureCanvas" style="display:none; width:100%; max-height:200px; border:1px solid #ccc;"></canvas>
              </div>
          `;
  ...
  function saveData() {
      ...
      } else if (signatureCanvas && signatureCanvas.getContext('2d').getImageData(0, 0, signatureCanvas.width, signatureCanvas.height).data.some(channel => channel !== 0)) {
      ...
      if (currentSignatureCellId && signatureData) {
          ...
          cell.innerHTML = signatureData.startsWith('data:image/png') ? '<img src="' + signatureData + '" alt="Captured Signature" style="max-width: 100px; max-height: 50px;">' : signatureData;
      }
  ...
  function showSignatureInput() {
      document.getElementById('signatureInputContainer').classList.remove('hidden');
 ...
 function saveData() {
  ...
  document.querySelectorAll('tbody tr').forEach(row => {
      ...
      row.id = `row-<span class="math-inline">\{row\.closest\('div'\)\.querySelector\('h3'\)\.innerText\.split\(' '\)\[0\]\.toLowerCase\(\)\}\-</span>{Array.from(row.parentNode.children).indexOf(row)}`;
  });
 </script>
